...
G=[80]#[  0 , 16 , 32 , 48 , 64 , 80 , 96, 112, 128, 144, 160, 176, 192, 208, 224, 240, 255]
# print(G)
def holo(G):
    G=int(G)
    print(G)
    ...
    rgb_image[:, :, 2] = np.clip(arr_r_modified + im_blank_array[:, :, 2], 0, 255)
...
# This is for diagnal binary phase grating.
def holo_D(G):
    G=int(G)
    print(G)
    im_blank=Image.new("RGB",(width,width))     # Square image
    pixels=im_blank.load()
    stripe_width = 24 # period
    spacing=stripe_width*2   # 2 times period
        ...
        for x in range(width):
            for i in range(0, height, spacing):
                for j in range(stripe_width):
                    if i+j<height:
                        pixels[x,i+j]=(0,0,G)
        im_blank.save(f"{file_n}_{G}(D)_p{spacing}.png")
