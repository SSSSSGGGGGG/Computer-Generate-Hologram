...
lambda_r = 0.532e-6  # Red wavelength
center_h=height//2
center_w=width//2
"""RGB lens"""
...
arr_r_modified=arr_r_mod*(255/2.63)
...
#     rgb_image[:, :, 1] = np.clip(arr_r_modified + im_blank_array[:, :, 1], 0, 255)
...
def holo_H(G):
    ...
    if reminder_h==0:
        ...
        for x1 in range(height):
            ...
            while i2 < reminder_h-1:
                ...
                pixels_h[i2+1+(round(loop_h)-1)*stripe_width,x1]=(0,0,0) 
                # print(i2+1+(round(loop_w)-1)*stripe_width)
                i2+=1
    im_blank_array = np.array(im_blank_h)
    ...
    # Convert `rgb_image` back to a PIL image and save it
    rgb_image_pil = Image.fromarray(rgb_image, 'RGB')
    rgb_image_pil.save(f"{file_n}_{G}(H)_p{spacing}.png")
for i in range(len(G)):
    holo_H(G[i])
