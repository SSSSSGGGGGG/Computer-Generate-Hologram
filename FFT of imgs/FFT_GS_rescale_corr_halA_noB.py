...
for i in range(iterations):
    ...
    current_field_r_n = im_shift_r *np.exp(1j * target_phase_constraint)#im_shift_r * 
    current_field_g_n = im_shift_g *np.exp(1j *target_phase_constraint)#im_shift_g *  
    current_field_b_n = im_shift_b *np.exp(1j * target_phase_constraint)#im_shift_b * 
...
im_modi.save(f"{filename}_GS {iterations}_l_cor_HA_noB.png")