...
for i in range(iterations):
    ...
    current_field_r_i = ifft2(ifftshift(im_shift_r*np.exp(1j * np.angle(current_field_r))))
    current_field_g_i = ifft2(ifftshift(im_shift_g*np.exp(1j * np.angle(current_field_g))))
    current_field_b_i = ifft2(ifftshift(im_shift_b*np.exp(1j * np.angle(current_field_b))))
    # # Inverse Fourier Transform to initial plane
    # current_field_r_i = ifft2(im_shift_r*np.exp(1j * np.angle(current_field_r)))
    # current_field_g_i = ifft2(im_shift_g*np.exp(1j * np.angle(current_field_g)))
    # current_field_b_i = ifft2(im_shift_b*np.exp(1j * np.angle(current_field_b)))
...
im_modi.save(f"{filename}_GS {iterations}_lens_5.png")