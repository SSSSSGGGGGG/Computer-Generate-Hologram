...
for n1 in r1:
    ...
    for n2 in r2:
        ...
        factor=1.7e2 # Small value for seeing noise.
        I1_rgb=np.zeros_like(holo1)
        I1_rgb[:, :, 0] = I1_r_normalized*(factor**2)
        I1_rgb[:, :, 1] = I1_g_normalized*(factor**2)
        I1_rgb[:, :, 2] = I1_b_normalized*(factor**2)
        # print(f"max {np.max(I1_rgb)}")
        # Save the reconstructed intensity.
        plt.figure()
        plt.imshow(I1_rgb, vmax=max_I)
        # plt.colorbar()
        plt.axis("off")
        ...
        M_rgb[:, :, 0] = np.abs(Reconstruction_holo1_r)*factor
        M_rgb[:, :, 1] = np.abs(Reconstruction_holo1_g)*factor
        M_rgb[:, :, 2] = np.abs(Reconstruction_holo1_b)*factor
        # print(f"max {np.max(M_rgb)}")
        plt.figure()
        plt.imshow(M_rgb,vmax=max_M)
        # plt.colorbar()
        plt.axis("off")
...
# I_o[:, :, 0] = original2_r*(factor**2)
# I_o[:, :, 1] = original2_g*(factor**2)
# I_o[:, :, 2] = original2_b*(factor**2)
# plt.figure()
...
# M_o[:, :, 0] = np.sqrt(original2_r)*factor
# M_o[:, :, 1] = np.sqrt(original2_g)*factor
# M_o[:, :, 2] = np.sqrt(original2_b)*factor

# plt.figure()
# plt.imshow(M_o,vmax=max_M)
# plt.axis("off")
# # plt.colorbar()
