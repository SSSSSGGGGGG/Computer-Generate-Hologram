...
for i in range(iterations2):
    ...
    current_field_b_i_t =np.sqrt(n*abs(current_field_b_i)**2/np.max(abs(current_field_b_i)**2)) *np.exp(1j * np.angle(current_field_b_i))
    print(f"turn {i}: max I={np.max(abs(current_field_r_i)**2)}, I={abs(current_field_r_i[250,250])**2}, Nor={abs(current_field_r_i[250,250])**2/np.max(abs(current_field_r_i)**2)}")
    # print(f"turn {i}: sum I={np.sum(abs(current_field_r_i)**2)}")
    ...
    print(f"turn {i}: max I={np.max(abs(current_field_r_n)**2)}, I={abs(current_field_r_n[250,250])**2}, Nor={abs(current_field_r_n[250,250])**2/np.max(abs(current_field_r_n)**2)}")
    ...
    # plt.figure()
    # plt.imshow(np.angle(current_field_r), cmap="hsv")
    # plt.colorbar()
    # plt.show()
...
def crop(im_modify,name):
    ...
    im_modi.save(f"{filename}_GS_n2_{iterations2}_{name}.png")

C_noL=crop(im_modify_noL, f"nl")

end_t=time.time()
print(f"Time consuming {end_t-start_t}s, iteration {iterations2}")