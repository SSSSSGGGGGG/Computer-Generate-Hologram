...
im_crop_nr=im_crop/np.sum(im[:,:,0])
#R channel
...
nw_field_r_if = ifftshift(ifft2(ifftshift(np.exp(1j * np.angle(nw_field_r)))))
...
plt.figure()
plt.plot(I_nw_crop_nr[hn//2,:])
plt.show()
# plt.figure()
# plt.imshow(I_nw_crop_nr, cmap="hot")
# plt.colorbar()
# # plt.savefig("I_nw_crop_nr.png", dpi=300, bbox_inches='tight')
# plt.show()
...
for i, v in enumerate(n):
    ...
    w_field_r_if = ifftshift(ifft2(ifftshift(np.exp(1j * np.angle(w_field_r)))))
...
# plt.colorbar()
# plt.show()
Diff_nw=im_crop_nr-I_nw_crop_nr
Diff_w=im_crop_nr-I_w_crop_nr
print(f"Diff_nw={np.sqrt(np.sum(Diff_nw))}, Diff_w={np.sqrt(np.sum(Diff_w))}, sum_o_cp={np.sum(im_crop_nr)},sum_nwcp={np.sum(I_nw_crop_nr)},sum_wcp={np.sum(I_w_crop_nr)}")

plt.figure()
plt.plot(I_w_crop_nr[hn//2,:])
plt.show()

# plt.figure()
# plt.imshow(I_w_crop_nr, cmap="hot")
# plt.colorbar()
# plt.savefig("I_w_crop_nr.png", dpi=300, bbox_inches='tight')
# plt.show()

# plt.figure()
# # plt.plot(n, sum_win_w_nr, label="I in the window with amplitude noise")
# plt.plot(n, sum_win_w, label="I (window) with n")
# plt.plot(n, sum_r_w_nr, label="I (circle) with n ")
# plt.axhline(y=sum_r_n_nr, color='r', linestyle='--', label="I (circle) without n")
# plt.axhline(y=sum_nw, color='b', linestyle='--', label="I (window) without n")
# plt.xlabel("Noise amplitude (n)")
# plt.ylabel("Total Intensity")
# plt.legend()
# plt.grid(True)
# plt.savefig("I vs noise.png", dpi=300, bbox_inches='tight')
# plt.show()

# upper=[np.max(I_nw),np.max(I_w)]
# upperlimit=np.max(upper)
# plt.figure()
# plt.imshow(I_nw,vmax=upperlimit, cmap="hot")
# plt.colorbar()
# plt.show()

# plt.figure()
# plt.imshow(I_w,vmax=upperlimit, cmap="hot")
# plt.colorbar()
# plt.show()
...
# plt.figure()
# plt.imshow(abs(nw_field_r_if), cmap="hot")
# plt.colorbar()
# plt.show()

# plt.figure()
# plt.imshow(abs(w_field_r_if)/0.00005, cmap="hot")
# plt.colorbar()
# plt.show()
