...
for n1 in r1:
    ...
    # n2=40-n1
        ...
        print(f"n1 {n1} n2 {n2}:min={np.max(I1_r_normalized),np.max(I1_g_normalized),np.max(I1_b_normalized)}")
        ...
        plt.close() 
        # plt.show()
        print(f"max D={np.max(diff_r1+diff_g1+diff_b1)},minD={np.min(diff_r1+diff_g1+diff_b1)}")
        ...
        plt.figure()
        plt.imshow(diff_r1+diff_g1+diff_b1,vmin=min_Diff,vmax=max_Diff,cmap="seismic")
        plt.colorbar()
