...
im=plt.imread(f"{filename}.png")[..., :3]
...
power1=2
#R
im_shift_r=fftshift(im)
#G
im_shift_g=fftshift(im)
#B
im_shift_b=fftshift(im)
...
def crop(im_modify,name):
    ...
    im_modi.save(f"{filename}_G_GS_{iterations,name}_input_{power1}_p2_{power2}.png")
