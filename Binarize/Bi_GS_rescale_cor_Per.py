...
def binarize(optimized_phase,period):# Stripe pattern parameters
    optimized_phase_a = np.zeros_like(optimized_phase)
    stripe_width_r = period  # Period
    spacing_r = stripe_width_r * 2  # Spacing (2x period)
    # Calculate loop counts and remainders
    loop_h = height / stripe_width_r
    reminder_h = height % stripe_width_r
    
    for x in range(width):
        for i in range(0, height, spacing_r):
            for j in range(stripe_width_r):
                if i + j < height:
                    optimized_phase_a[i + j, x] = 1 if optimized_phase[i + j, x] >= 0 else 0
    
    # Handle width remainder (additional stripes for non-even divisions)
    if reminder_h != 0:
        for x in range(width):
            for i in range(0, height, spacing_r):
                for j in range(stripe_width_r):
            ...
            while i2 < reminder_h-1:
                    # print(i2+1+(round(loop_h)-2)*stripe_width_r)
                    optimized_phase_a[i2+1+(round(loop_h)-1)*stripe_width_r, x] = 1 if optimized_phase[i2+1+(round(loop_h)-2)*stripe_width_r, x] >= 0 else 0
                    i2+=1
    return optimized_phase_a

optimized_phase_r_o = np.angle(current_field_r)
optimized_phase_r=binarize(optimized_phase_r_o,35)
phase_rr_modi = optimized_phase_r * 184


optimized_phase_g_o = np.angle(current_field_g)
optimized_phase_g = binarize(optimized_phase_g_o,27)
...
optimized_phase_b = binarize(optimized_phase_b_o,24)
...
def crop(im_modify,name):
    ...
    im_modi.save(f"Bi_{filename}_GS_{iterations}_l_C_{name}_P.png")
