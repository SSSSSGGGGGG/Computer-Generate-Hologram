...
def FFT(im,type_):  
    ...
    if type_=="tri":
            gr=gg=gb=1.361*np.pi
    else:
            gr,gg,gb=np.pi,np.pi,np.pi
    
    r_gray=optimized_phase_r_bi*gr
    
    g_gray=optimized_phase_g_bi*gg
    
    b_gray=optimized_phase_b_bi*gb
    return r_gray,g_gray,b_gray
t="tri"
v_phase_r,v_phase_g,v_phase_b=FFT(im_v,t)
h_phase_r,h_phase_g,h_phase_b=FFT(im_l,t)
plt.figure()
plt.imshow(abs(ifftshift(ifft2(np.exp(1j*h_phase_r)))))
# h_phase_r_con,h_phase_g_con,h_phase_b_con=FFT(im_l_con)
h_phase_r_fl,h_phase_g_fl,h_phase_b_fl=FFT(im_l_fl,t)
