...
current_field_r = fftshift(fft2(im_shift_r ))
current_field_g =fftshift(fft2(im_shift_g))
current_field_b =fftshift(fft2(im_shift_b))
...
# optimized_phase_r = np.arctan(2.65*np.cos(optimized_phase_r_o))
# phase_rr_modi=(optimized_phase_r/np.pi+1)*(255/1)
# phase_rr_modi_mod=np.mod(phase_rr_modi,255)

# optimized_phase_g_o = np.angle(current_field_g)
# optimized_phase_g = np.arctan(2.65*np.cos(optimized_phase_g_o))
...
# optimized_phase_b = np.arctan(2.65*np.cos(optimized_phase_b_o))
...
phase_rr_modi=(optimized_phase_r_o/np.pi+1)*(255/1)
phase_rr_modi_mod=np.mod(phase_rr_modi,255)

# optimized_phase_g_o = np.angle(current_field_g)
# # optimized_phase_g = np.where(optimized_phase_g_o < 0, 0, 1)
# phase_gr_modi=(optimized_phase_g_o/np.pi+1)*(255/2.63)
# phase_gr_modi_mod=np.mod(phase_gr_modi,255)

# optimized_phase_b_o = np.angle(current_field_b)
# # optimized_phase_b = np.where(optimized_phase_b_o < 0, 0, 1)
# phase_br_modi=(optimized_phase_b_o/np.pi+1)*(255/3.55)
# phase_br_modi_mod=np.mod(phase_br_modi,255)
...
im_modify_noL = phase_rr_modi_mod
...
def crop(im_modify,name):
    ...
    im_modi.save(f"Bi_{filename}_{iterations}_C_{name}.png")
# R=crop(im_modify_r, "r")
# G=crop(im_modify_g, "g")
# B=crop(im_modify_b, "b")
# C=crop(im_modify_noL, "c")
# C_noL=crop(im_modify_noL, "nL")
plt.imshow(im_modify_noL,cmap="hsv")
plt.axis("off")
plt.show()
# plt.imsave("Tri_CGH_h.png", im_modify_noL,cmap="hsv")
plt.imsave("CGH_o.png", im_modify_noL,cmap="hsv")