...
stripe_width = 92 # period
stripe_width_v = 200 # period
    ...
    for x1 in range(height):
        for i1 in range(0, stripe_width):
            ...
            for k in range(int(loop)):
                ...
                pixels_h[i1+k*stripe_width,x1]=(0,0,int((i1)* interval))
    im_blank_h.save(f"{file_n}(blazed)_p{stripe_width}_{color_value}.png")
else:
    interval=color_value/(stripe_width-1)
    for x1 in range(height):
        for i1 in range(0, stripe_width):
        ...
        while i2 < reminder-1:
            ...
            pixels_h[i2+1+(round(loop)-1)*stripe_width,x1]=(0,0,int((i2)* interval)) 
    ...
    for x1 in range(width):
        for i1 in range(0, stripe_width_v):
            ...
            for k in range(int(loop_v)):
                # print(i1+k*stripe_width)
                pixels[x1,i1+k*stripe_width_v]=(0,0,int((i1)* interval))
    im_blank.save(f"{file_n}(blazed)V_p{stripe_width_v}_{color_value}.png")
else:
    interval=color_value/(stripe_width_v-1)
    for x1 in range(width):
        for i1 in range(0, stripe_width_v):
        ...
        while i2 < reminder_v-1:
            # print(round(loop))
            pixels[x1,i2+1+(round(loop_v)-1)*stripe_width_v]=(0,0,int((i2)* interval)) 
